...
%macro calculate1_initial (skunumber) ;
	data temp1 ;
		set r_&model..Cat_&category._&skunumber ;
		if horizon= &horizon ;
		ape= ae/actual ;
		...
		sku=&skunumber ;
		category="&category" ;
		keep sku horizon ape sape ase ae category ;
		
	run ;
	data temp_all_&horizon._&model ;
		set temp1 ;
	run ;
%mend ;
%macro calculate1 ( skunumber) ;
	...
	run ;
	data temp_all_&horizon._&model ;
		set temp_all_&horizon._&model  temp1 ;
	run ;
%mend ;
...
%macro d1 (model, measure) ;
	%do horizon = 1 %to 1 ;
		data temp_wk_&horizon (rename= (&measure = &model._&measure._&horizon )) ;
			set Temp_all_&horizon._&model ;
			keep &measure ;
		run ;
		data temp_all_&model._m&measure ;
			set temp_wk_&horizon ;
		run ;
		proc means data= temp_all_&model._m&measure mean noprint;
			var &model._&measure._&horizon ;
			output out=x_&horizon mean=mean ;
		run ;
		data x_&horizon ;
			set x_&horizon ;
			horizon = &horizon ;
	...
	%do horizon = 2 %to 12 ;
		...
		run ;
		data temp_all_&model._m&measure ;
			merge temp_all_&model._m&measure temp_wk_&horizon ;
		run ;
		proc means data= temp_all_&model._m&measure mean noprint ;
			var &model._&measure._&horizon ;
			output out=x_&horizon mean=mean ;
	...
	%do horizon = 2 %to 12 ;
			data x_all_&model._m&measure   ;
				set x_all_&model._m&measure x_&horizon (rename= (mean=m_&model._&measure))  ;
				 
				keep m_&model._&measure horizon ;
			run ;
	%end ;
