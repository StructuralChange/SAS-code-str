...
%macro extract_var_name (data) ;
	...
	data test_sales ;
		set &data ;  
		t=_n_ ;
		if _n_>165 then delete ; /* our evaluation focus on week 1- week 165 */
		keep units_1 - units_3000 t ; 
	run ;
	data test_price ;
		set &data ;  
		t=_n_ ;
		if _n_>165 then delete ;
...
/* step 2.1: calculate the stats for sales for each SKU */
%macro run_candidate_model_s   ;
	...
	run ;
%mend ;
/* step 2.1: calculate the stats for price for each SKU */
%macro run_candidate_model_pr   ;
	...
	run ;
%mend ;
/* step 2.1: calculate the stats for feature for each SKU */
%macro run_candidate_model_f   ;
	...
	run ;
%mend ;
/* step 2.1: calculate the stats for display for each SKU */
	...
	%do cc= 1 %to &nobs ;
		...
		/* outliers ;*/
			...
			data t2 ;
				...
				z= q3-q1 ;
				if q1-1.5*z<units_&skunumber<q3+1.5*z  then outliers=0 ;
					else outliers= 1 ;
			...
			data trend (rename= (units_&skunumber= linear_trend)) ;
				set x1 ;
				if _name_ ="t" ;
				keep units_&skunumber ;
			run ;
...
run ;
 ...
 data expl_cat.stat_all_allcat2  ;
 	set  expl_cat.stat_all_allcat  ;
 		 if category= "spagsau" and skunumber = 176 then delete ; 
		 if category= "cigets" and skunumber= 614 then delete ; 
		 if category= "toothbr" and skunumber= 536 then delete ; 
		 if category= "saltsnc" and skunumber= 1530 then delete ; 
		 
		 n_of_sku_count= 1 ; /* we can use pivot table 'count' to measure how many SKUs are included in the category */
run ;
