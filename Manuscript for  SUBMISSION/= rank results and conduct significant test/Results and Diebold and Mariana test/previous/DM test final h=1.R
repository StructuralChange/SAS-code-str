...
#1. test for MAE 
      ...
      test1  <- unlist(c(overall_r_adl4_versus_r_ad4_ew["ae_0"]), use.names=FALSE) #6 ad4_ew   
      test2  <- unlist(c(overall_r_adl4_versus_r_ad4_ew["ae_1"]), use.names=FALSE) #6 ad4_ew 
...
#3. test for MAPE 
      f1  <- unlist(c(overall_r_own2_versus_r_base["forecast_original"]), use.names=FALSE)      #1 base
      f2  <- unlist(c(overall_r_own2_versus_r_base["forecast_sb_adjusted"]), use.names=FALSE)   #2 own2
...
#5. test for MSE 
      ...
      e1  <- unlist(c(overall_r_own2_versus_r_base["ae_0"]), use.names=FALSE)   #1 base
      e2  <- unlist(c(overall_r_own2_versus_r_base["ae_1"]), use.names=FALSE)   #2 own2
      e3  <- unlist(c(overall_r_own2_versus_r_adl4["ae_1"]), use.names=FALSE)   #3 adl4
...
#5. test for Mean relative AE
      ...
      f1  <- unlist(c(overall_r_own2_versus_r_base["forecast_original"]), use.names=FALSE)      #1 base
      f2  <- unlist(c(overall_r_own2_versus_r_base["forecast_sb_adjusted"]), use.names=FALSE)   #2 own2, #forecasts by own2, as the reference
      f3  <- unlist(c(overall_r_own2_versus_r_adl4["forecast_sb_adjusted"]), use.names=FALSE)   #3 adl4
      ...
      as.bigz(prod(rel_1))
      as.bigz((prod(rel_1))^(1/length(rel_1)))
      
      sq <- 1/length(rel_1)
      
      3333333333333333333333333333333333^sq
      
    
      test1 <- rel_4[8940:38920]
      (prod(test1))^(1/length(test1))
      ...
      prod(rel_1)^(1/length(rel_1))
      library(gmp)
      avg <- 1
      for (i in 1:length(rel_1)) {
        avg <- avg*rel_1[i]
...
dm.test(rel_1,rel_2,h=12,power=1)  
dm.test(rel_2,rel_3,h=12,power=1)

dm.test(rel_2,rel_4,h=12,power=1)
dm.test(rel_2,rel_5,h=12,power=1)

dm.test(rel_3,rel_6,h=12,power=1)
dm.test(rel_3,rel_7,h=12,power=1)
dm.test(rel_3,rel_8,h=12,power=1)#### issue



class(dm.test(rel_1,rel_2,h=12,power=1) )
results <- (dm.test(rel_1,rel_2,h=12,power=1) )
class(results$p.value)
...
#overall_r_own2_versus_r_ow2_ic <- read.csv("overall_r_own2_versus_r_ow2_ic.csv")
 ...
 #we evluate other models' performance 'relative' to own2
      ...
      mae0 <- aggregate(overall_r_adl4_versus_r_ad4_ic$ae_0, by=list(overall_r_adl4_versus_r_ad4_ew$category, overall_r_adl4_versus_r_ad4_ew$sku), mean)
      mae1 <- aggregate(overall_r_adl4_versus_r_ad4_ic$ae_1, by=list(overall_r_adl4_versus_r_ad4_ew$category, overall_r_adl4_versus_r_ad4_ew$sku), mean)
      class(mae1)
