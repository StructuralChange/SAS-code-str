...
/* note that, the variable list is , i.e., &catstring_in_use */
 %macro selection  ;
	...
	run ;
	 /* NON include the 'own-variables' because they are important and may be wrongfully excluded by the algorithms ;
	add own variables ;*/
	data temp_own_x ;
		length EffectEntered $ 16;
		EffectEntered= "" ;
...
/* this macro extract the variables which we obtained from the 1st stage VSM process.
	the variables are from library data_v0n.___.
...
%macro kk ;
	...
	%do kk= 1 %to 1 ;
		...
		/* %let tt= 143 ;*/
		data temp1_&tt ;
			set data_v0n.Vs_&start._&end._&category._all ;
			keep r&tt ;
			if r&tt="" then delete ;
		run ;
		data temp2_&tt ;
			set data_v2n.Vs_&start._&end._&category._all ;
			keep r&tt ;
			if r&tt="" then delete ;
		run ;
	...
	data data_v3n.Vs_&start._&end._&category._all ;
		set  tempall ;
	run ;
