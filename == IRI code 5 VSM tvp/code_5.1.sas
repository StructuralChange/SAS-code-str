...
%mend ;
 ...
 %macro selection  ;
	...
	run ;
	 /* we will NOT include the 'own-variables' because  as this is for TVP, if we do, the tvp model may not converge;*/
...
%do ii= 1 %to 1 ;
	%let skunumber= &&sku_for_modelling&ii ; %let output_var= Data_t1.vs_&start._&end._&category._all ;
	%selection ;

%end ;
%do ii= 2 %to &nobs ;
	%let skunumber= &&sku_for_modelling&ii ; %let output_var= Data_t1.vs_&category._&skunumber ;
	%selection ;
	data Data_t1.vs_&start._&end._&category._all ;
		merge Data_t1.vs_&start._&end._&category._all Data_t1.vs_&category._&skunumber ;
	run ;
	proc delete data= Data_t1.vs_&category._&skunumber ;
	run ;
%end ;
%mend ;
...
/* check */
%obsnvars(Data_t1.vs_&start._&end._&category._all) ;
