...
/* test sig for h=4, for all, not only for sb period */
%let horizon=12 ;
...
data overall_sig_test_all_h&horizon ;
	set temp1 ;
run ;
...
*/
data aa ;
	set expl_str.overall_&model0._versus_&model1 ; /* here we choose the all_results not only with structural breaks */
...
data overall_sig_test_all_h&horizon ;
	set overall_sig_test_all_h&horizon temp1 ;
run ;
