...
149	150	151	152	153	154	155	156
  ...
  /* we leave 5% of the sample untested, e.g., we conduct the Chow test for the central 95% of observations in the sample 
	e.g., we leave 5%*120= 7 observations, so we conduct the test from obs 5 to obs 117  */
...
%do round= 1 %to %eval(&length-&ewc_window) %by &ewc_jump ;
/* %let round = 1 */
	...
	proc model data= dataset_temp_sales_excluded ; 
	    ...
	    fit logs_&skunumber 
	            ...
	            / hccme=1      outresid  outall
								  out=model_adl 
								  outest=parms ;
		  		range week= %eval(&start+2+&round) to &end ;
 ...
 * step 3.2:	The post-sample forecasts are in "forecast" ; 
	data forecast ;
		set forecast (rename=(logs_&skunumber= forecast)) ;
		if week>&end ;
		 ;
	...
	data forecast&round (rename=(forecast=forecast&round)) ;
		set forecast ;
	run ;
...
%end ;
	...
	data forecast_all ;
		set forecast1 ;
			forecast_sum=forecast1 ;
	run ;

	%macro merge1 ;
	%do t= %eval(1+ &ewc_jump)  %to %eval(&length-&ewc_window)  %by &ewc_jump  ;
		data  forecast_all ;
			merge forecast_all forecast&t ;
			forecast_sum=  forecast_sum+  forecast&t ;
			by week ;
		proc delete data= forecast&t ;
		run ;
	%end ;
	%mend ;
	...
	data forecast ;
		set forecast_all ;
		forecast=  forecast_sum/ %eval((&length-&ewc_window)/&ewc_jump)	;
		keep week forecast ;
	run ;
...
/* step 3.1: product the dataset with absolute error for the no-change model */
	data dataset0 ;
		...
		logs_&skunumber= exp(logs_&skunumber) ;
		keep week logs_&skunumber f_&skunumber d_&skunumber promoall_&skunumber ;
	run ;
