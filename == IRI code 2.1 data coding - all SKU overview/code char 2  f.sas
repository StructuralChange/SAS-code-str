...
%include 'Q:\= IRI data research\== External macros\code x count rows and columns.sas' ; run ;
 ...
 /* this macro calcualtes the number of SKUs on feature for each category across the 208 weeks, and

		 				  the number of SKUs on feature for each category for each week
...
/* count how the number of active and non-slow movers change over time for each category */
 ...
 %macro rebuild ( category) ;
		...
		data temp_all ;
			set temp_all ;
			i= _n_ ;
			retail_list= compress(_name_,"f_");
	...
	/* extract the data for each SKU with enough valid data ;*/
		%let a= &&retained_valid_sku&k ; 
		 data temp_&a ;
			set data1c.Data_groc_&category ;
			  if f_&a=1  then count_&a= 1 ;  else count_&a= 0 ;
				 keep  count_&a ;
	     run ;
...
%macro rebuild2_0 (category)  ;
	...
	data tt (rename =(active_all= active_all_&category))  ;
		...
		keep active_all ;
	run ;
	data Data1c.count_all_cat_ot_f ;
		set tt ;
	run ;
...
%macro rebuild2 (category)  ;
	...
	run ;
	data Data1c.count_all_cat_ot_f ;
		merge Data1c.count_all_cat_ot_f tt ;
	run ;
...
%macro rebuild3_0 (category) ;
	proc means data= data1c.count_all_cat_ot_f mean std noprint ;
		var active_all_&category ;
		output out= stat_&category mean= valid_sku_mean_f std= valid_sku_std_f ;
	run ;
	data Data1c.stat_all_cat_f ;
		set stat_&category ;
		category= "&category" ;
	run ;
%mend ;
%macro rebuild3 (category) ;
	...
	run ;
	data Data1c.stat_all_cat_f ;
		set Data1c.stat_all_cat_f stat_&category ;
	run ;
%mend ;
