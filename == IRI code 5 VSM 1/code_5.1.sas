...
%mend ;
 ...
 %macro selection  ;
	...
	data &output_var (rename=(EffectEntered= r&skunumber)) ;
		set  Lasso temp_own1 temp_own2 temp_own3    ;
...
%do ii= 1 %to 1 ;
	%let skunumber= &&sku_for_modelling&ii ; %let output_var= data_v1.vs_&start._&end._&category._all ;
	%selection ;

%end ;
%do ii= 2 %to &nobs ;
	%let skunumber= &&sku_for_modelling&ii ; %let output_var= data_v1.vs_&category._&skunumber ;
	%selection ;
	data data_v1.vs_&start._&end._&category._all ;
		merge data_v1.vs_&start._&end._&category._all data_v1.vs_&category._&skunumber ;
	run ;
	proc delete data= data_v1.vs_&category._&skunumber ;
	run ;
%end ;
%mend ;
...
/* check */
%obsnvars(data_v1.vs_&start._&end._&category._all) ;
